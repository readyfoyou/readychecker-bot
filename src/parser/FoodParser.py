...
class SilpoParser(BaseParser):
	async def parse(self) -> tuple:
		price_element = self.soup.find('span', attrs={'data-autotestid': 'product-main-price'})
		old_pr_el = self.soup.find('del', attrs={'data-autotestid': 'product-price-old'})
		name_el = self.soup.find("h1", attrs={'data-autotestid': 'product-page__title'})
		discount = self.soup.find('span', class_='sale-price__discount')
		if price_element:
			price_text = price_element.text
			del_elements = price_text.replace(' ', '')
			match = re.search(r'\d+\.?\d*', del_elements)
